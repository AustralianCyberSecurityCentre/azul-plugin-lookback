...
RUN pip install \
    ...
    azul-plugin-lookback==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-lookback==$(cd /tmp/src && python -m setuptools_scm);fi
...
ENTRYPOINT ["azul-plugin-lookback-hash"]